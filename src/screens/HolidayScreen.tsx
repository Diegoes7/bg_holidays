...
const HolidayScreen: React.FC<Props> = ({ holiday, onNavigate }) => {
  return (
    ...
    >
      <div style={{ ...styles.container, borderTop: `8px solid ${holiday.color}` }}>
        <button style={styles.backButton} onClick={() => onNavigate('calendar')}>
          ⬅ Към календара
        </button>
        <h1 style={{ ...styles.header, color: holiday.color }}>{holiday.title}</h1>
        <p style={styles.date}>{holiday.date} - Национален празник</p>

        <motion.div className="card" style={styles.card} whileHover={{ scale: 1.02 }}>
